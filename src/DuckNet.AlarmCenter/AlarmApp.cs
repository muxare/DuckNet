...
public static class AlarmApp
{
    public static WebApplication Create(string[] args, AlarmOptions? options = null)
    {
        ...
        db.Write((conn, tx) => AlarmStore.EnsureLastAlarmEventIdColumn(conn, tx));
        ...
        app.MapPost("/alarms/{duckId}/resolve", (string duckId, KernelDb kernelDb, AlarmStore store) =>
        {
            ...
            var ok = kernelDb.Write((conn, tx) => store.TryResolve(conn, tx, duckId, traceId));
            return ok
                ? Results.Ok(new { duckId, status = "resolved" })
                : Results.NotFound();
        });
