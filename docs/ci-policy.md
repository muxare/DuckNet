...
- **Advisory (backlog):** four chains over GitHub issues, none of which closes, relabels, or rewrites an issue.
  - `backlog-build.yml` — `workflow_dispatch` only. Builds epics and stories from the description/vision documents and/or the code (either alone suffices), then an independent Sonnet session checks every candidate against the repo. `already-done`, `unfounded`, and anything under `0.6` confidence are reported and never filed. Re-runs update by `<!-- ducknet-backlog:KEY -->` marker, then title; closed matches are skipped. The build job never touches issues: it ends at a schema-validated plan (`backlog-actions.json`) uploaded as an artifact and rendered into the job summary. ~`$1.50`–`$2.50` per run.
  - `backlog-apply.yml` — the only thing that files a built backlog, and it runs no model. Called by `backlog-build.yml` (`apply: true`, the default) or dispatched by hand with any earlier build run's `run_id`. Either way it pauses on the **`backlog-apply` GitHub Environment** until a reviewer approves, then downloads that run's plan artifact and files it. Approving therefore approves the exact model output in the summary — re-dispatching the build would have produced *different items* from a fresh model session. First step refuses to run if the environment carries no protection rules, because GitHub silently creates an unprotected environment for a name it has never seen. **One-time setup:** Settings → Environments → New environment `backlog-apply` → Required reviewers. ~`$0`.
  - **Stale plans reconcile rather than clobber.** A plan pins issue numbers, open/closed state, and spliced bodies at build time; all three move while it waits for approval. [`reconcile-backlog-plan.py`](../.github/scripts/reconcile-backlog-plan.py) re-derives them at apply time. `plan-backlog-issues.py` is a pure function of `(final.json, issue dump, sha, run_url)`, so apply first *replays* it over the build-time dump: reproducing the approved plan byte for byte proves nothing frozen moved — not the chain output, not the planner's code, not the commit in the issue footers, not the artifact. It then re-runs the same function against a fresh dump and applies that. The approval fixes *what* is filed (frozen model output); *where* it lands is re-resolved against current facts. An update to a since-closed issue becomes a skip, a create that would now duplicate becomes an update, and text a human added after the plan was built survives the re-splice. Every move is reported in the job summary; a key set or title that moves is a hard failure that files nothing.
