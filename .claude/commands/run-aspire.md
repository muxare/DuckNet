...
Run the Step 9 multi-Center demo (team-shared, human-triggered).
...
6. Optional poison: `POST` Telemetry `/bus/poison`, then `GET` Alarm or Dashboard `/dlq`. Replay with `POST /dlq/{id}/replay?fix=true` or skip with `POST /dlq/{id}/skip`.
7. Optional: stop `alarm` in Aspire, wait for a squeak burst, start it again — `/alarms` on AlarmCenter should fill in from the log.
8. Mixed v1/v2 replay is `dotnet test --filter MixedVersion`, not a live Aspire flag.
9. Kernel before/after: `--hot-demo --shard-count 1` vs `--shard-count 3`.
