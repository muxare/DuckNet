...
**Tech:** .NET 9, one solution, .NET Aspire AppHost from step 4 onward. Start with in-memory transport behind an `IEventBus` abstraction; swap the transport late without touching Center code — that swap is itself proof the architecture is right.
